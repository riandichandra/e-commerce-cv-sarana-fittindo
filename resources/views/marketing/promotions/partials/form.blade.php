...
<div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
    ...
    <div>
        <label for="type" class="text-sm font-bold text-texthighlight">Type</label>
        <select id="type" name="type" class="mt-2 w-full border-gray-300 text-sm shadow-sm focus:border-primary focus:ring-primary" required>
    ...
    <div>
        <label for="value" class="text-sm font-bold text-texthighlight">Value</label>
        <input id="value" name="value" type="number" step="0.01" min="0" value="{{ old('value', $promotion->value) }}"
            class="mt-2 w-full border-gray-300 text-sm shadow-sm focus:border-primary focus:ring-primary" required>
        <x-input-error :messages="$errors->get('value')" class="mt-2" />
    </div>

    <div>
        <label for="min_purchase" class="text-sm font-bold text-texthighlight">Minimum Purchase</label>
        <input id="min_purchase" name="min_purchase" type="number" step="0.01" min="0" value="{{ old('min_purchase', $promotion->min_purchase) }}"
    ...
    <div>
        <label for="max_discount" class="text-sm font-bold text-texthighlight">Maximum Discount</label>
        <input id="max_discount" name="max_discount" type="number" step="0.01" min="0" value="{{ old('max_discount', $promotion->max_discount) }}"
            class="mt-2 w-full border-gray-300 text-sm shadow-sm focus:border-primary focus:ring-primary">
...
<div class="mt-7 flex items-center justify-end gap-3">
    <a href="{{ route('marketing.promotions.index') }}" class="bg-gray-200 px-4 py-2 text-sm font-bold text-gray-700 hover:bg-gray-300">
        CANCEL
    </a>
