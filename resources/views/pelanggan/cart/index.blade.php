<x-pelanggan-layout>
    <section class="bg-[#071d33] px-8 py-14 text-white">
        <div class="mx-auto max-w-[1290px]">
            <p class="text-xs font-black uppercase tracking-[.24em] text-[#c8d8ee]">Keranjang Pelanggan</p>
    ...
    <section class="bg-[#f7faff] px-8 py-14">
        <div class="mx-auto max-w-[1290px]">
            @if (session('success') || session('error'))
                <div
                    class="mb-6 border-l-4 {{ session('success') ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-500 bg-red-50 text-red-700' }} px-4 py-3 text-sm font-semibold">
                ...
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
                    <div class="space-y-4">
                        @foreach ($cart->items as $item)
                            <div
                                class="grid grid-cols-1 gap-5 bg-white p-5 shadow-sm md:grid-cols-[120px_1fr_auto] md:items-center">
                                <a href="{{ route('pelanggan.products.show', $item->product) }}"
                                    class="block h-28 overflow-hidden bg-[#dce8f7]">
                                ...
                                <div>
                                    ...
                                    <p class="mt-2 text-sm font-bold text-[#c8102e]">Rp
                                        {{ number_format($item->product->price, 0, ',', '.') }}</p>
                                    <p class="mt-1 text-xs text-gray-500">Stok tersedia: {{ $item->product->stock }}</p>
                                    @if ($item->quantity > $item->product->stock)
                                        <p class="mt-1 text-xs text-red-600">Jumlah di keranjang melebihi stok saat ini.
                                        </p>
                                    @endif
                                </div>

                                <div class="flex items-center gap-3 md:justify-end">
                                    <form action="{{ route('pelanggan.cart.update', $item) }}" method="POST"
                                        class="flex items-center gap-2">
                                        @csrf
                                        @method('PATCH')
                                        <input type="number" name="quantity" value="{{ $item->quantity }}"
                                            min="1" max="{{ max($item->product->stock, 1) }}"
                                    ...
                                    <form action="{{ route('pelanggan.cart.destroy', $item) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="flex h-10 w-10 items-center justify-center bg-red-50 text-red-600 hover:bg-red-100"
                    ...
                    <aside class="h-fit bg-white p-6 shadow-sm">
                        ...
                        <a href="{{ route('pelanggan.cart.checkout') }}"
            ...
            @else
                <div class="bg-white px-6 py-16 text-center">
                    ...
                    <a href="{{ route('pelanggan.products.index') }}"
                        class="mt-7 inline-flex h-11 items-center bg-[#c8102e] px-6 text-xs font-black uppercase tracking-[.16em] text-white hover:bg-[#9f0d24]">
                        Lihat Produk
                    </a>
                </div>
            @endif
