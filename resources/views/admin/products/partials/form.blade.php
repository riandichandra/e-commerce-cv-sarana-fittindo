...
{{-- <div class="flex flex-col gap-1">
    ...
    <textarea name="specifications_text" id="specifications_text" rows="7"
        ...
        class="border border-gray-300 p-2 focus:ring-primary focus:border-primary transition w-full">{{ old('specifications_text', collect($product?->specifications ?? [])->map(fn ($value, $key) => is_int($key) ? $value : $key . ': ' . $value)->implode("\n")) }}</textarea>
    <p class="text-xs text-gray-500">Gunakan format <strong>Nama: Nilai</strong>, satu spesifikasi per baris.</p>
