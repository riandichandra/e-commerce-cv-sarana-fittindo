<x-admin-layout>
    <div class="flex flex-col gap-2">
        ...
        <div class="w-full overflow-hidden rounded-md border border-gray-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 bg-[#FFF7F8] px-5 py-4">
                <div>
                    <h2 class="font-bold tracking-wider text-texthighlight">LIST PROMOSI</h2>
                    <p class="mt-1 text-sm text-gray-500">Admin hanya dapat melihat data promosi yang dibuat marketing.</p>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse ($promotions as $promotion)
                            ...
                            <tr class="align-top transition hover:bg-gray-50">
                                <td class="px-5 py-4 font-semibold text-gray-500">{{ $promotions->firstItem() + $loop->index }}</td>
                                <td class="px-5 py-4">
                                    ...
                                    <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                                        ...
                                        @if ($promotion->banner_image)
                                            <a class="font-semibold text-primary hover:text-primary-dark" href="{{ asset('storage/' . $promotion->banner_image) }}" target="_blank">
                                                Lihat banner
                                            </a>
                                        @endif
                                    </div>
                                    @if ($promotion->description)
                                        <p class="mt-2 max-w-md text-sm leading-5 text-gray-500">{{ $promotion->description }}</p>
                                    @endif
                                </td>
                                <td class="px-5 py-4">
                                    <div class="font-semibold text-texthighlight">{{ $promotion->type === 'percent' ? 'Persen' : 'Nominal' }}</div>
                                    ...
                                    @if ($promotion->max_discount)
                                        <div class="mt-1 text-xs text-gray-500">Maks. Rp {{ number_format($promotion->max_discount, 0, ',', '.') }}</div>
                                    @endif
                                </td>
                            ...
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center">
                                    <div class="mx-auto flex max-w-sm flex-col items-center gap-2 text-gray-500">
                                        ...
                                        <p class="text-sm">Data promosi akan tampil di sini setelah dibuat oleh marketing.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
