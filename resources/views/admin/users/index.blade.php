    ...
    <div class="flex flex-col gap-2">
        ...
        <div class="bg-[#FFF1F3] p-5 w-full">
            <div class="flex items-center justify-between">
                <h2 class="font-semibold tracking-wider text-texthighlight">SEMUA PENGGUNA</h2>
                <form method="GET" action="{{ route('admin.users.index') }}">
                    <select name="role_id" onchange="this.form.submit()"
                        class="border border-gray-300 bg-white px-3 py-2 text-sm">
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            <tr class="border-b border-gray-200 text-sm">
                                <td class="py-3 px-3">{{ $users->firstItem() + $loop->index }}</td>
                                ...
                                <td class="py-3 px-3">
                                    <span class="inline-flex bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-6 px-3 text-center text-sm text-gray-500">
                                    Belum ada pengguna.
                                </td>
                            </tr>
                        @endforelse
