<x-admin-layout>
    <div class="flex flex-col gap-6">
        <div class="flex items-center gap-1 text-xs">
            ...
            <p>></p>
            <p class="font-bold tracking-wider text-primary">BRANDS</p>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($brands as $brand)
                            <tr class="text-sm transition hover:bg-[#fff7f8]">
                                <td class="px-5 py-4 align-top font-semibold text-gray-500">{{ $brands->firstItem() + $loop->index }}</td>
                                <td class="px-5 py-4">
                                    <p class="font-bold text-texthighlight">{{ $brand->name }}</p>
                                    <p class="mt-1 text-xs text-gray-500">{{ $brand->slug }}</p>
                                    @if ($brand->description)
                                        <p class="mt-1 max-w-xl text-xs text-gray-500">{{ \Illuminate\Support\Str::limit($brand->description, 90) }}</p>
                                    @endif
                                </td>
                                <td class="px-5 py-4">
                                    <span class="inline-flex items-center gap-1.5 bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-700">
                                        <iconify-icon icon="mdi:package-variant-closed"></iconify-icon>
                                        {{ number_format($brand->products_count, 0, ',', '.') }} produk
                                    </span>
                                </td>
                                <td class="px-5 py-4">
                                    <span class="px-2.5 py-1 text-xs font-bold {{ $brand->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600' }}">
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-14 text-center">
                                    <div class="mx-auto flex max-w-md flex-col items-center">
                                        ...
                                        <p class="mt-2 text-sm text-gray-500">Tambahkan merek untuk melengkapi informasi produk.</p>
