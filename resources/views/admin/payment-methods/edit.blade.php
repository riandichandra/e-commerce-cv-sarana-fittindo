<x-admin-layout>
    ...
    <div class="bg-[#FFF1F3] p-5 w-full">
        <h2 class="font-semibold tracking-wider text-texthighlight">EDIT BANK ACCOUNT</h2>
        <form action="{{ route('admin.payment-methods.update', $paymentMethod) }}" method="POST" class="mt-4 w-full flex flex-col gap-4">
            @csrf
            @method('PUT')
